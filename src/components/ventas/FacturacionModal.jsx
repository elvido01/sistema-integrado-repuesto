            ...
            >
                ...
                <div className="grid grid-cols-12 gap-4">
                  <div className="col-span-12 md:col-span-8 bg-white p-4 rounded-md shadow-sm">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      ...
                      <div className="lg:col-span-2"><Label>Dirección</Label><p className="p-2 bg-gray-100 rounded-md text-sm min-h-[38px]">{cliente.direccion}</p></div>
                      <div><Label>Teléfonos</Label><p className="p-2 bg-gray-100 rounded-md text-sm min-h-[38px]">{cliente.telefono}</p></div>
                      <div><Label>Crédito</Label><p className="p-2 bg-gray-100 rounded-md text-sm min-h-[38px]">{cliente.autorizar_credito ? `Disponible (${cliente.dias_credito} días)` : 'No Autorizado'}</p></div>
                    </div>
                  </div>
                  <div className="col-span-12 md:col-span-4 bg-white p-4 rounded-md shadow-sm">
                    ...
                    <div className="space-y-4">
                      <div className="flex gap-2">
                        <div className="w-20"><Label>Nº</Label><p className="p-2 bg-gray-100 rounded-md text-sm text-center font-bold">NUEVA</p></div>
                        ...
                        <TableRow className="sticky top-12 bg-white z-10">
                          ...
                          <TableCell colSpan={9}>
                            <Button variant="outline" className="w-full justify-start" onClick={() => setIsProductSearchOpen(true)}>
                              <Search className="mr-2 h-4 w-4" /> F6 - Buscar producto por descripción...
                            </Button>
                          </TableCell>
                        </TableRow>
                        {items.map((item) => (
                ...
                <div className="grid grid-cols-12 gap-4 mt-4">
                  <div className="col-span-12 lg:col-span-4 bg-white p-4 rounded-md shadow-sm">
                    ...
                    <RadioGroup value={paymentType} onValueChange={setPaymentType} className="flex gap-4 mb-4">
                      ...
                      <div className="flex items-center space-x-2"><RadioGroupItem value="credito" id="credito" disabled={!cliente.autorizar_credito} /><Label htmlFor="credito">Crédito</Label></div>
                    </RadioGroup>
                    {paymentType === 'credito' && (
              ...
              <footer className="sticky bottom-0 bg-gray-200 px-6 py-3 flex items-center justify-between rounded-b-md border-t">
                ...
                <div className="flex gap-4">
                  <Button variant="outline" onClick={onClose}><XCircle className="w-4 h-4 mr-2" />ESC – Salir</Button>
                  <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={onConfirm} disabled={isSaving}><Save className="w-4 h-4 mr-2" />{isSaving ? 'Guardando...' : 'F10 – Grabar'}</Button>
                </div>
              </footer>
            </motion.div>
          </motion.div>
