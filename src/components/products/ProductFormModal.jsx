...
const ProductFormModal = ({ isOpen, onClose, onSave, product }) => {
  ...
  const populateForm = useCallback(async (p) => {
    setFormData({
      ...
      tipo_id: p.tipo_id ? String(p.tipo_id) : null,
      marca_id: p.marca_id ? String(p.marca_id) : null,
      modelo_id: p.modelo_id ? String(p.modelo_id) : null,
      suplidor_id: p.suplidor_id ? String(p.suplidor_id) : null,
