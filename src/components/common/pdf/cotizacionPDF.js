...
export const generateCotizacionPDF = (cotizacion, cliente, details) => {
  ...
  doc.text(displayNombre, 14, 88);
  doc.text(`RNC: ${displayCliente.rnc || ''}`, 14, 96);
  doc.text(displayCliente.direccion || '', 14, 104);
  doc.text(`Tel: ${displayCliente.telefono || ''}`, 14, 112);

  // Cotizacion Info
  doc.setFont('helvetica', 'bold');
  doc.text("FECHA COTIZACIÓN:", 140, 88);
  doc.text("VÁLIDA HASTA:", 140, 96);
  doc.setFont('helvetica', 'normal');
  doc.text(formatDate(cotizacion.fecha_cotizacion), 185, 88);
  doc.text(formatDate(cotizacion.fecha_vencimiento), 185, 96);
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(formatCurrency(cotizacion.total_cotizacion), 200, totalsY + 50, { align: 'right' });
