...
| `hermes_whatsapp_conversaciones` / `hermes_whatsapp_mensajes` (vistas) | una fila por chat con `sin_responder`, `horas_desde_cliente`, último mensaje; mensajes con quien='yo'/'cliente' | Detectar chats sin responder y leer la conversación | Lee ✓ | **Ya creadas.** El teléfono viene del JID real del chat (extensión v2); si un chat viejo sigue con telefono NULL, su metadata trae `apta_crm=false` → NO crear ficha CRM de ese chat |
