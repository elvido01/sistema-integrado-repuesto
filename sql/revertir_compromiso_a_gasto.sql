-- =====================================================================
-- REVERTIR: los compromisos en efectivo vuelven a contarse como
-- COMPROMISO (no como gasto). Se descarta el enganche a gastos_diarios
-- (duplicaba el egreso en el cierre: linea "Compromisos (Efectivo)" +
-- "Gastos Diarios").
--
-- Deja todo como antes de compromiso_efectivo_a_gasto.sql:
--   * get_caja_excedente_dashboard vuelve a restar TODOS los compromisos
--     pagados (excedente) y los pagados HOY en efectivo (caja del dia).
--   * borra los gastos_diarios que se habian creado por compromiso
--     (columna compromiso_id) para no duplicar.
--
-- El pago en efectivo ahora: imprime comprobante (front) y aparece en el
-- cierre como "Compromisos (Efectivo)". Idempotente. Correr en PRODUCCION.
-- =====================================================================

-- 1) Deshacer los gastos creados por compromiso (backfill + enganche).
DELETE FROM public.gastos_diarios WHERE compromiso_id IS NOT NULL;

-- 2) Restaurar el dashboard de caja (version de fix_caja_dia_pagos_efectivo).
...
DO $$ BEGIN
  IF to_regprocedure('public.registrar_migracion(text)') IS NOT NULL THEN
    PERFORM public.registrar_migracion('revertir_compromiso_a_gasto.sql');
  END IF;
END $$;

NOTIFY pgrst, 'reload schema';

-- Verificacion: no deben quedar gastos ligados a compromisos
SELECT COUNT(*) AS gastos_por_compromiso_restantes
FROM public.gastos_diarios WHERE compromiso_id IS NOT NULL;
