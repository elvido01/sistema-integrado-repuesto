...
DO $$
DECLARE
  -- En PL/pgSQL los nombres NO distinguen mayúsculas: `T` y `t` eran la
  -- misma variable y el bloque no compilaba. Todo con prefijo v_.
...
BEGIN
  ...
  PERFORM pg_temp.anotar(3, 'Una conversación entrega de a uno, el más viejo primero',
                         '1 | prueba 3 primero', n || ' | ' || COALESCE(v_txt, '(ninguno)'));
  ...
  PERFORM pg_temp.anotar(6, 'Tras 3 fallos sale de la cola y conserva el motivo',
                         '0 tomados | error | 3 | fallo 3', n || ' tomados | ' || v_txt);
  ...
  PERFORM pg_temp.anotar(13, 'chat_estado cambia el detalle sin mover el estado',
                         'procesando | consultando el catálogo', v_txt);
