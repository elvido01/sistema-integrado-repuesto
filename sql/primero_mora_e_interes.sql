-- =====================================================================
-- NO SE ABONA AL CAPITAL CON INTERES O MORA PENDIENTE
-- ---------------------------------------------------------------------
-- La regla, dicha por el dueño el 2026-08-17:
--   "A ningun cliente se le puede abonar al capital si tiene interes
--    pendiente o mora pendiente."
--
-- LA REGLA YA EXISTIA... A MEDIAS
--   registrar_pago_prestamo tiene DOS caminos:
--
--   1) Distribucion automatica (cuota mas vieja primero) — SI la respeta:
--        ab_mora := LEAST(v_restante, rec.mora_pend);     -- 1o mora
--        ab_int  := LEAST(v_restante, rec.interes_pend);  -- 2o interes
--        ab_cap  := LEAST(v_restante, rec.capital_pend);  -- 3o capital
--
--   2) Abonos marcados fila por fila (p_abonos) — NO la respetaba.
--      Y esa es justo la que usa la pantalla del Recibo de Pago: aplicaba
--      exactamente lo que el cajero tecleaba, sin mirar el orden.
--
--   Por ahi entraba el problema.
--
-- QUE ARREGLA ESTO, Y POR QUE ES LA CURA DE RAIZ
--   La mora y el interes corriente no son montos guardados: son formulas
--   que se recalculan sobre LO QUE QUEDA PENDIENTE por los dias de atraso.
--   Al bajar el capital, bajan ellos tambien — y lo ya generado se esfuma.
--
--   Ejemplos medidos en la cartera (PT-0024777 cuota 1, pendiente 6,696.32,
--   64 dias, mora 4% = 563.59):
--     A) abona 300 de mora y NADA de capital
--          pendiente sigue igual -> formula da 563.59 - 300 = 263.59 QUEDA  OK
--     B) abona 300 de mora Y 1,436.41 de capital
--          pendiente baja        -> formula da 442.70 - 300 = 142.70
--          deberia quedar 263.60           SE ESFUMAN 120.90
--     C) paga la cuota COMPLETA y 300 de mora
--          pendiente 0           -> formula da 0.00
--          deberia quedar 263.60           SE ESFUMAN 263.60   el peor
--
--   Con la regla puesta, B y C no pueden ocurrir: para tocar el capital hay
--   que haber cubierto antes toda la mora y todo el interes. Cuando el
--   capital baja, ya no queda nada que esfumarse. No hay que "cuajar" nada
--   ni cambiar la formula: el orden de cobro lo resuelve.
--
--   Casos reales que esto habria impedido:
--     TEODORA B. PEREZ GIL, recibo 0147707 — abono 3,200 de capital a cuotas
--       con 81-87 dias de atraso sin tocar la mora: se borraron 353.10.
--     DANNY SEVERINO GUERRERO, recibo 0147780 — abono capital con interes
--       corriente pendiente: se borraron 110.47.
--
-- LO QUE NO CAMBIA (sigue siendo decision de la empresa)
--   * El interruptor de mora del cliente: apagado -> mora_pendiente = 0, y
--     entonces no bloquea nada. La empresa sigue pudiendo no cobrar mora.
--   * La tasa de mora (cliente / prestamo / default de la empresa).
--   * Al volver a encender la mora, se recalcula sobre el monto pendiente y
--     los dias de atraso a la fecha de hoy — tal cual funciona ahora.
--   * Perdonar por nota de credito.
--   Esta regla NO decide cuanto se cobra. Solo el ORDEN en que se cobra.
--
-- OJO CON EL MOSTRADOR
--   A partir de esto, un cliente muy atrasado no podra abonar al capital sin
--   cubrir antes su mora e interes. Si eso traba un cobro real, la salida es
--   la de siempre y sigue estando en manos de la empresa: bajarle la tasa,
--   apagarle la mora, o perdonarla con nota de credito.
--
-- Base: sql/fix_pago_interes_corriente.sql (la version viva). El unico
-- cambio es el bloque "REGLA" antes de grabar. Idempotente / re-ejecutable.
-- =====================================================================
...
BEGIN
  ...
  IF p_abonos IS NOT NULL AND jsonb_array_length(p_abonos) > 0 THEN
    ...
    v_int_pend := COALESCE((v_estado->>'intereses_pendientes')::numeric, 0);
    ...
    IF v_ab_cap > 0 AND (v_falta_int > 0.01 OR v_falta_mora > 0.01) THEN
      RAISE EXCEPTION
        'No se puede abonar al capital: quedan % de interes y % de mora sin cubrir. Cobre primero el interes y la mora.',
...
DO $mig$ BEGIN
  IF to_regprocedure('public.registrar_migracion(text)') IS NOT NULL THEN
    PERFORM public.registrar_migracion('primero_mora_e_interes.sql');
  END IF;
END $mig$;
