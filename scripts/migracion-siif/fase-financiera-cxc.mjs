...
// --force: des-aplicar PRIMERO los abonos/NC que la app aplicó sobre cuotas
// migradas (violarían la FK al borrar las cuotas). El viejo es el libro oficial.
if (FORCE) {
  const idSet = new Set(ids);
  const desaplicar = async (tabla) => {
    const rows = [];
    for (let from = 0; ; from += 1000) {
      const { data, error } = await supabase.from(tabla).select('id, cuota_id').range(from, from + 999);
...
// Recibos de ingreso del día para el CIERRE DE CAJA (solo pagos nuevos
// de esta corrida con fecha de los últimos 7 días).
{
