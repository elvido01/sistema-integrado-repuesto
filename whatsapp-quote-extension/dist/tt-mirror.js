// MotoFlow Omni — puente TikTok → Sales Hub.
//
// Escucha lo que anuncia tt-probe.js y lo manda a la RPC omni_mirror_tiktok.
// Vive en el mundo aislado del content script, que es el único que puede
// leer chrome.storage (donde está la sesión) y hablar con Supabase sin que
// la página se entere.
//
...
// TikTok. Si nunca ha entrado al panel, este puente se queda callado.
//
// Es hermano de ig-mirror.js y hace la mitad de lo que hace aquel: los
// mensajes entran, pero desde el CRM todavía no se contesta. Ver la
// explicación en tt-probe.js.
(() => {
  'use strict';
  ...
  // Huella del hilo: si no cambió nada, no se vuelve a mandar. La bandeja de
  // TikTok se refresca sola cada pocos segundos y sin esto el espejo estaría
  // llamando a la base todo el rato para no escribir nada.
  ...
  const enviar = async () => {
    ...
    if (!(await refrescarCredenciales())) {
      // Sin sesión de MotoFlow no hay nada que hacer: se descarta en
      // silencio para no acumular conversaciones en memoria indefinidamente.
      pendientes.clear();
      return;
    }
    ...
    for (const hilo of lote) {
      ...
      try {
        const r = await fetch(`${config.url}/rest/v1/rpc/omni_mirror_tiktok`, {
          method: 'POST',
          headers: {
      ...
      } catch { /* el espejo nunca debe estorbar el uso normal de TikTok */ }
    }
  };
  ...
  window.addEventListener('message', (ev) => {
    ...
    const d = ev.data;
    if (!d || d.source !== 'motoflow-omni' || d.type !== 'tt-threads') return;

    for (const hilo of d.hilos || []) {
      if (!hilo?.thread_id || !hilo?.messages?.length) continue;
